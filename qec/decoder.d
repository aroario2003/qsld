...
struct Decoder {
    ...
    private int[][][string] map_syndrome_to_error_vector() {
        ...
        int[][][string] syndrome_to_error_map;
        ...
        foreach (ps; pauli_strings) {
            ...
            string syndrome_str = syndrome.elems.map!(x => x.to!string).array.join;
            syndrome_to_error_map[syndrome_str] ~= err_vec.elems;
        }
    ...
    private bool compare_syndrome(Vector!int original_syndrome, string generated_syndrome) {
        int[] gen_syndrome_bits = generated_syndrome.map!(c => (c == '1') ? 1 : 0).array;
        for (int i = 0; i < gen_syndrome_bits.length; i++) {
            if (original_syndrome.elems[i] != gen_syndrome_bits[i]) {
                return false;
            }
        }
        return true;
    ...
    // Converts the pauli string generated by the decoder into the corresponding error vector
    private Vector!int convert_pauli_string_to_error_vector(string error_pauli_string) {
        char[] pauli_arr = error_pauli_string[].dup;
        Vector!int err_vec = Vector!int(2 * this.qc.num_qubits, new int[2 * this.qc.num_qubits]);

        int err_vec_idx = 0;
        for (int i = (cast(int) pauli_arr.length) - 1; i >= 0; i--) {
            if (pauli_arr[i] == 'X') {
                err_vec[err_vec_idx] = 1;
            } else if (pauli_arr[i] == 'Y') {
                err_vec[err_vec_idx] = 1;
                err_vec[this.qc.num_qubits + err_vec_idx] = 1;
            } else if (pauli_arr[i] == 'Z') {
                err_vec[this.qc.num_qubits + err_vec_idx] = 1;
            }
     ...
     */
    string decode(Vector!int original_syndrome, Vector!int original_error_vec) {
        int[][][string] syndrome_to_error_map = map_syndrome_to_error_vector();
        Vector!int err_vec;
        string error_pauli_string;
        foreach (generated_syndrome, errors; syndrome_to_error_map) {
            if (compare_syndrome(original_syndrome, generated_syndrome)) {
                foreach (error; errors) {
                    if (error == original_error_vec.elems) {
                        err_vec = Vector!int(2 * this.qc.num_qubits, error);
                        error_pauli_string = convert_error_vector_to_pauli_string(err_vec);
                        return error_pauli_string;
                    }
                }
                auto error_vec_list = syndrome_to_error_map[generated_syndrome];
                err_vec = Vector!int(2 * this.qc.num_qubits, error_vec_list[$ - 1]);
                error_pauli_string = convert_error_vector_to_pauli_string(err_vec);
                break;
            }
        }
    ...
    void correct_state(string error_pauli_string) {
        ...
        this.qc.qec_conf.qec_mode = "none";
        ...
        this.qc.qec_conf.qec_mode = "automatic";
    }
}
